...
class ZKUserSaveView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            data = json.loads(request.body)
            users = data.get('users', [])  # Expecting list of user objects with user_id, name, card_no
            if not users:
                return JsonResponse({
                    'success': False,
...
class ZKDeviceAllOperationsView(FormView):
    """View for handling all ZKTeco device operations in a single page with tabs."""
    template_name = 'zk_device/device_all_operations.html'
    form_class = ZKDeviceSyncForm  # Reused for device sync and user sync
    connection_form_class = ZKDeviceConnectionTestForm  # For test connection

    def get_context_data(self, **kwargs):
        """Add context for all tabs and forms."""
        context = super().get_context_data(**kwargs)
        context.update({
            'title': _("All Device Operations"),
            'subtitle': _("Manage ZKTeco device connections, data sync, and user integration"),
            'zk_available': ZK_AVAILABLE,
            'connection_form': self.connection_form_class(self.request.POST or None),
            'sync_form': self.form_class(self.request.POST or None),
            'active_tab': self.request.POST.get('active_tab', 'test-connection'),
        })
        return context

    def post(self, request, *args, **kwargs):
        """Handle POST requests for all operations based on active tab."""
        active_tab = request.POST.get('active_tab')
        context = self.get_context_data()

        if not ZK_AVAILABLE:
            messages.error(request, _("ZK library not available. Please install it with 'pip install pyzk'"))
            return self.render_to_response(context)

        if active_tab == 'test-connection':
            return self._handle_test_connection(request, context)
        elif active_tab == 'sync-devices':
            return self._handle_sync_devices(request, context)
        elif active_tab == 'save-attendance':
            return self._handle_save_attendance(request)
        elif active_tab == 'sync-users':
            return self._handle_sync_users(request, context)
        elif active_tab == 'save-users':
            return self._handle_save_users(request)
        else:
            messages.error(request, _("Invalid operation"))
            return self.render_to_response(context)

    def _handle_test_connection(self, request, context):
        """Handle device connection test."""
        form = self.connection_form_class(request.POST)
        if form.is_valid():
            device = form.cleaned_data['device']
            test_result = self._test_device_connection(device)
            context.update(test_result)
            context['active_tab'] = 'test-connection'
            return self.render_to_response(context)
        context['connection_form'] = form
        return self.render_to_response(context)

    def _handle_sync_devices(self, request, context):
        """Handle device attendance data sync."""
        ...
        if form.is_valid():
            ...
            sync_result = self._sync_devices(devices, start_date, end_date)
            context.update(sync_result)
            context['active_tab'] = 'sync-devices'
            return self.render_to_response(context)
        context['sync_form'] = form
        return self.render_to_response(context)

    def _handle_save_attendance(self, request):
        """Handle saving attendance data to the database."""
        try:
            data = json.loads(request.body)
            attendance_data = data.get('attendance_data', [])
            if not attendance_data:
            ...
            for record in attendance_data:
                try:
                    ...
                    device = get_object_or_404(ZKDevice, id=record['device_id'])
                    timestamp = datetime.strptime(record['timestamp'], '%Y-%m-%d %H:%M:%S')
                    ...
                    errors.append(f"Error saving record for user {record.get('user_id', 'unknown')}: {str(e)}")
                    continue
    ...
    def _handle_sync_users(self, request, context):
        """Handle user data sync from devices."""
        form = self.form_class(request.POST)
        if form.is_valid():
            devices = form.cleaned_data['devices']
            users_data = []
            for device in devices:
                try:
                    ...
                    if conn:
                        ...
                        conn.disconnect()
                        messages.success(request, f"Synced {len(users_data)} users from {device.name}")
                except Exception as e:
                    logger.error(f"Error syncing users from {device.name}: {str(e)}")
                    messages.error(request, f"Error syncing {device.name}: {str(e)}")
            context.update({
                'users_data': users_data,
                'sync_performed': True,
                'active_tab': 'sync-users'
            })
            return self.render_to_response(context)
        context['sync_form'] = form
        return self.render_to_response(context)

    def _handle_save_users(self, request):
        """Handle saving synced users to Employee table."""
        try:
            data = json.loads(request.body)
            ...
            for user in users:
                ...
                try:
                    ...
                    errors.append(f"Error saving user {user_id}: {str(e)}")
            ...
            return JsonResponse({
                ...
                'skipped_count': skipped_count,
                'errors': errors,
                'message': f'Saved {saved_count} users, {skipped_count} existed, {len(errors)} errors'
            })
        except json.JSONDecodeError:
            return JsonResponse({'success': False, 'error': _('Invalid JSON data')}, status=400)
        except Exception as e:

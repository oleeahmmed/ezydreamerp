        ...
        try:
            ...
            for user_id in user_ids:
                try:
                    ...
                    # 👉 Get the first device used for this user
                    first_device = user_attendance_data.first().device if user_attendance_data.first() else None
                    device_location = first_device.name if first_device else 'Unknown Location'

                    # 👉 Create/get department named after device location
                    device_department, _ = Department.objects.get_or_create(
                        code=f'DEVICE_{first_device.id}' if first_device else 'UNKNOWN',
                        defaults={
                            'name': device_location,
                            'description': f'Department for device location {device_location}'
                        }
                    )
                    ...
                    employee = Employee.objects.create(
                        ...
                        department=device_department,  # 🔥 Set department from device location
                        designation=default_designation,
                        joining_date=joining_date,
                        basic_salary=0.00,
                        is_active=True,
                        location=device_location  # 🔥 New location field
                    )
